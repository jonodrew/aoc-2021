...
def get_non_nine_neighbours(coord: Tuple[int, int]) -> List[Tuple[int, int]]:
    ...
    return list(
        filter(
            lambda new_coord: grid()[new_coord[1]][new_coord[0]] != 9, neighbour_coords
        )
    )
...
def calculate_total_risk_levels(minima: FrozenSet[Tuple[int, int]]) -> int:
    return sum(map(lambda x: 1 + x, (grid()[y][x] for x, y in minima)))


def find_basin_size(
    points_to_check: Iterator[Tuple[int, int]],
    basin_size: int = 0,
    already_checked: FrozenSet = frozenset(),
) -> int:
    try:
    ...
    points_to_check = itertools.chain(
        points_to_check, iter(get_non_nine_neighbours(next_point))
    )
    all_points = filter(
...
def find_three_largest_and_get_product() -> int:
    return functools.reduce(
        operator.mul, sorted(find_all_basin_sizes(), reverse=True)[:3]
    )
